...
runs one managed Godot editor session on the project, held to the active task's Git branch, launches
the game through that editor's own debug adapter, streams editor output into the UI, and persists
compressed run logs.
...
active task branch", and a repository with no commits failed Tauri's `setup` — which panics, so
...
Tasks remain independently resumable. Git projects automatically receive a `gofer/task-*` branch
with its base, head, and merge commits recorded. Every task works in the project's own checkout;
there is one, and Godot's import cache is shared by all of them, so opening a task reimports only
the files that branch actually changes.

Opening a task therefore moves the working tree, which is a sequence rather than a pointer update.
The editor session stops first — Godot holds every open scene in memory, never rereads one a
checkout changed underneath it, and would save that stale copy over the branch being switched to —
then anything left loose is committed onto the branch being left, and only then does the checkout
move and the current-task pointer follow it. A turn in flight refuses the switch outright. The merge
action visits the base branch, merges, moves the task branch onto the result, and returns the user
to the task they pressed it on.
...
The Gofer editor addon ships inside the Rust binary and is copied into the project as
...
`.gitignore`. Unstaging puts `project.godot` back byte for byte, down to how it ended: a checkout is
refused outright when a tracked file is modified and the branch being moved to changed it too, so a
stray newline left by cleanup would be a task the user could not switch away from.

A cleanup ledger in Gofer's application-data directory records what was introduced and what existed
beforehand, and it is written before the project is touched, so a crashed session is repairable.
...
through a package manager: the Godot documentation and source the transports were written against,
Monaco, and the pinned `gdformat` sidecar. Dependencies installed by npm and Cargo carry their
licences in `package-lock.json` and `src-tauri/Cargo.lock`.
...
`npm run build` is the application, not a gate. It runs `build:frontend` and then Cargo, and writes
the binary the desktop app actually runs — `src-tauri/target/release/gofer`. Building the frontend
alone leaves that binary untouched, so a change you can only see in a release build needs `build`.
...
(`/tmp/gofer-live-run`), so a sweep cannot touch your projects.

Run `test:live:reset` between sweeps. A sweep leaves its data directory and its `gofer/task`
branches in place on purpose — a failed sweep is worth opening afterwards — but a second sweep
started on top of the first reopens the first one's tasks and reports on those.
