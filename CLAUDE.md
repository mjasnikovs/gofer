...
Astryx v0.5.2 · 163 components CLI: run every command as `npx astryx <cmd>` (shown below as
`astryx ...`).
...
// BAD — a block. Every line of it is a name the code should have had.
// First we collect the open scenes.
// Then we filter out the ones the addon does not own.
// Then we sort them so the active one is first.

// GOOD — justifies a deviation nobody should undo
// filesystem_changed fires before the import thread finishes; scan() completing
// is the only edge where the imported files are actually readable.
```
