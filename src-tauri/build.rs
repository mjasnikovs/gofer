...
fn main() {
    ...
    tauri_build::try_build(
        tauri_build::Attributes::new()
            ...
            // No `.commands(...)`. All it ever did was write an `allow-$command` permission per
            // name into `permissions/autogenerated/`, and no capability has ever referenced one of
            // those identifiers: `permissions/main-window-commands.toml` grants the whole surface
            // as a single set, and `capabilities/default.json` names that set. Two lists meant one
            // could fall behind — it had, by twenty-three commands and nine files nothing would
            // ever regenerate or delete. The permission set is the one list now, and
            // `the_window_is_granted_exactly_the_commands_it_registers` keeps it honest.
