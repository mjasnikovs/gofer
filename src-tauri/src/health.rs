...
fn git_checks(workspace: &Path, repository: &git::RepositoryStatus) -> Vec<HealthCheck> {
    ...
    if repository.has_commit && repository.tracks_editor_cache {
        checks.push(
            HealthCheck::new(
                "git-editor-cache",
                "Godot cache in history",
                HealthStatus::Degraded,
                "This repository is tracking Godot's .godot cache folder. Godot rewrites it every \
                 time the editor opens, so each task switch records a commit that contains no work \
                 — and a merged task starts asking to be merged again.",
            )
            .with_remedy(Remedy::new(
                RemedyAction::StopTrackingEditorCache,
                "Stop tracking the Godot cache",
                "Adds .godot/ to .gitignore and removes the folder from Git's history in one \
                 commit. Your game files are not touched. Godot rebuilds its cache once, the next \
...
pub fn apply(workspace: &Path, action: RemedyAction) -> Result<(), String> {
    match action {
        ...
        RemedyAction::StopTrackingEditorCache => git::untrack_editor_cache(workspace),
...
mod tests {
    ...
    #[test]
    fn a_tracked_godot_cache_is_reported_with_a_fix() {
        ...
        let cache = check_in(&checks, "git-editor-cache");
        assert_eq!(cache.status, HealthStatus::Degraded);
        assert_eq!(
            cache.remedy.as_ref().map(|remedy| remedy.action),
        ...
        assert!(clean.iter().all(|check| check.id != "git-editor-cache"));
    }

    fn check_in<'a>(checks: &'a [HealthCheck], id: &str) -> &'a HealthCheck {
        checks
            .iter()
            .find(|check| check.id == id)
            .unwrap_or_else(|| panic!("no {id} check in {checks:?}"))
    }
}
