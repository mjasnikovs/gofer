...
fn announce_design_session<R: Runtime>(app: &AppHandle<R>, session_id: &str, opening: bool) {
...
pub(crate) fn design_session<R: Runtime>(
    app: &AppHandle<R>,
    params: &Value,
    ...
    if params
        ...
        == Some(true)
    {
        return crate::ai_tools::probe(DESIGN_SESSION_TOOL);
    }
    ...
    announce_design_session(app, session_id, opening);
...
mod tests {
    ...
    fn a_design_loop_announces_both_of_its_edges_to_the_window() {
        ...
        announce_design_session(app.handle(), "design-1", true);
        announce_design_session(app.handle(), "design-1", false);
    ...
    fn a_design_session_is_routed_by_name_and_is_not_a_catalogue_domain() {
        ...
        for state in ["open", "closed"] {
            let answer = crate::ai_tools::dispatch(
                app.handle(),
                crate::ai_tools::ToolRequest {
    ...
    fn a_design_session_with_no_state_is_refused() {
        let app = unattended_app();
        let failure = crate::ai_tools::dispatch(
            app.handle(),
            crate::ai_tools::ToolRequest {
                tool: DESIGN_SESSION_TOOL.to_owned(),
