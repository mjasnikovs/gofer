...
/// The adapter an acceptance test launched itself, mirroring `script::bind_test_session`: the
/// supervisor starts a windowed editor, which a headless gate cannot run. Absent from every
/// non-test build.
#[cfg(all(test, feature = "godot-acceptance"))]
static TEST_SESSION: Mutex<Option<(u16, String)>> = Mutex::new(None);

#[cfg(all(test, feature = "godot-acceptance"))]
pub fn bind_test_session(dap_port: u16, worktree: &str) {
    disconnect();
    if let Ok(mut slot) = TEST_SESSION.lock() {
        *slot = Some((dap_port, worktree.to_owned()));
    }
}
...
/// The editor the debug commands bind to: the active session's adapter port and its worktree.
