...
/** The branch it was on, which is where the restore has to leave it. */
let baseBranch = ''
...
/**
 * Puts the project back to the commit the sweep started from, with none of its task branches left.
 *
 * The order matters: Git refuses to delete the branch `HEAD` points at, and a task *is* a branch —
 * the sweep ends on the last one it opened. So the checkout goes back to the commit it started from
 * first, and only then are the branches deleted.
 */
function restoreWorkspace() {
    // Worktrees from a sweep that ran before tasks became branches. Gofer makes none of its own now.
    ...
    // Back onto the branch the project was on, rather than left on a detached HEAD the next sweep
    // would branch its first task from.
