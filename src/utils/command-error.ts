 ...
 *
 * Every backend command rejects with a coded failure, and a check refuses one that does not, so
 * `command_failed` now means the rejection never reached a command rather than that a command had
