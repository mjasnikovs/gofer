...
/**
 * The panel as it reads while the editor is playing the project — and as it reads when it is not.
 *
 * Every field here describes a game: that one was launched, where it is stopped, the frames of that
 * stop, the variables in them. None of it survives the game, and the adapter does not always say so
 * — a debuggee killed from outside, or an editor that went away under it, sends nothing at all. So
 * the launch was believed until something contradicted it, and the toolbar offered Stop Game over a
 * game that had been gone for an hour, with a stack from it still on screen.
 *
 * Godot's own `is_playing_scene()`, polled by the addon and carried through the session, is what
 * says whether there is a game. Asked here, the whole reading expires with the thing it describes.
 */
