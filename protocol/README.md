...
A game the debugger has paused is running and unreachable at once, and it is the failure that looks
most like a slow one: the process is alive, the editor still reports a playing scene, and nothing
arrives. A break that lands while a launch is still waiting for the helper — a main scene whose
script does not parse is the everyday cause — ends that launch with `runtime_broke` (retryable), and
a request forwarded to a game that has said nothing since it was paused is refused with the same
code rather than waiting out its deadline. `runtime.get_state` reports it as `broke`. A break after
the helper has announced is left alone: that is a breakpoint doing its job, and the game goes on
answering once it is continued.
