...
export function apisPrompt(refined, {inventory, files, canSearch} = {}) {
    return (
        ...
        + 'Look every one of them up before you write it. Use docs_search.search for engine '
        + 'classes. For anything in this project ask the language server first — '
        + 'script.workspace_symbols finds a symbol you cannot place, script.references finds who '
        + 'calls it, and script.hover and script.signature_help give its exact signature — and '
        + 'fall back to read and grep only '
        + 'where it answers nothing. Do not write a signature from memory — a plausible wrong '
        + 'signature costs more than an absent one.\n\n'
...
export function toolingPrompt(goal) {
    return (
        ...
        + 'Those two cover importing and booting, and that is all the shell is for here. Anything '
        + 'about the game while it runs — a scene playing, a click, what the tree holds afterwards '
        + '— is read back through the godot tool’s runtime operations, so do not list a '
...
export function composePrompt(refined, research, answers) {
    return (
        ...
        + 'VERIFY\n'
        + 'A fenced block of checks that prove the work, one per line. Above each one, a `#` line '
        + 'naming what it proves — that name is what the user watches while it runs:\n'
        + '```sh\n'
        + '# <what this check proves>\n'
        + '<the check>\n'
        + '```\n'
        + 'A check is one of two things.\n'
        + 'A shell command from TOOLING, written as it is there.\n'
        + 'Or a call to the running game, written as the tool name and one JSON object on the same '
        + 'line:\n'
        + '  godot_runtime {"ops": [{"op": "run", "scene": "scenes/main.tscn"}, {"op": "wait", '
        + '"frames": 30}, {"op": "get_tree"}], "contains": "Bullet"}\n'
        + 'The ops run in order and reach the real game: `run` starts a scene, `input` sends the '
        + 'clicks and keys a player would, `wait` renders frames, and `get_tree` and `inspect_node` '
        + 'read back what is there. `contains` is the assertion — the text the answer has to hold, '
        + 'and without it the line only proves the call was answered. Reach for this before a check '
        + 'script: it needs no file, and it asserts on the game rather than on a print.\n'
        ...
        + 'When neither shape reaches what this task changed, and only then, a task may write its '
        + 'own check and run it as `godot --headless --audio-driver Dummy --script '
        + '.gofer/checks/<name>.gd`, which Gofer already keeps out of git. Paths are relative to '
        + 'the workspace; an absolute one is refused.\n'
