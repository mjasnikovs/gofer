...
export function apisPrompt(refined, {inventory, files, canSearch} = {}) {
    return (
        ...
        + 'For anything in this project ask the language server first — godot_script '
        + 'workspace_symbols finds a symbol you cannot place, references finds who calls it, and '
        + 'hover and signature_help give its exact signature — and fall back to read and grep only '
...
export function toolingPrompt(goal) {
    return (
        ...
        + '— is read back through the godot_runtime tool, so do not list a shell command for it.\n\n'
