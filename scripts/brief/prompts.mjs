...
export function toolingPrompt(goal) {
    return (
        ...
        + 'guessing conventional names. A command that does not exist is worse than no command. Most '
        + 'Godot projects define none at all, and `(none)` is the right answer for those.\n\n'
...
export function composePrompt(refined, research, answers) {
    return (
        ...
        + 'VERIFY\n'
        + 'A fenced block of shell commands that prove the work, one per line, each copied from the '
        + 'TOOLING section of the research:\n'
        + '```sh\n'
        + '<a command TOOLING lists>\n'
        + '```\n'
        + `TOOLING is the only source of a command. When it lists none — most Godot projects define `
        + `none — the block holds exactly \`${NO_COMMANDS}\` and nothing else. A command this project `
        + 'does not define is worse than no command: it is a step that always fails.\n\n'
